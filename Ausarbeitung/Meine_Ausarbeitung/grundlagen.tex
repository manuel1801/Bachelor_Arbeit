...
CNNs erweitern die in \ref{subsec:nn} beschriebenen
Neuronalen Netze um Convolutional Layer, welche vor den 
Fully Connected Layern ausgeführt werden.
Diese erhalten den Input als zwiedimensionale Matrix, 
und führen mathematische Faltungsoperationen darauf aus, 
wodurch bestimmte merkmale des Inputs herausextrahiert werden 
können.

Hauptanwendungsgebiet der CNNs ist die Bilderkennung,
in anderen bereichen wie ... kommen sie aber auch 
zum Einsatz.

Anstatt das alle Neuronen zweier benachbarter Schichten 
durch Gewichte/Parameter miteinander verbunden sind, 
stellen kleinere sog. Filter Matrizen, die 
Parameter dar.

Diese werden zeilenweise über das Input Bild 
geschoben und an jeder Stelle die 
mathematische Faltung angewendet.
...
Jedes Faltungsergebnis ergibt einen Wert der nächsten 
Schicht, die eine korellation zwischen input bild 
und filter darstellt.

Enthalten diese ähnlice muster, wie in ... dargestellt 
bsp horizonotale linien, wird diese merkmal
verstärkt in der Feature Map abgebildet.
...
durch die Faltung entsteht eine räumliche invarianz 
fpr das zu erkennende Objekt im input bild.
ein weitere forteil ist der geringere rechenaufwand
durch die gemeinsame nutzung der parameter.
...
Die Feature maps werden dann zur klassifizierung 
einem fully connected layer übergeben.
...
\end{figure}
\vspace{1cm}
die werte der filter matrizen werden über die 
Backpropagation für die jeweiligen klassen 
eingelernt.

Da diese, insbesondere in den vorderen Convolutional 
Layern für alle klassen sehr ähnlich sind, können vortrainierte
Filter verwendet werde, welche dann an den eigenen 
Datensatz angepasst werden, \textit{Transfer Learning}.
