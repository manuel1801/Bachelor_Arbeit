...
parser.add_argument(
    "--mode", help="Dataset category - train, validation or test", required=False)
parser.add_argument(
...
parser.add_argument("--inside", help="Include inside images",
                    required=False, type=int, default=0)
parser.add_argument("--max", help="max nr to download",
                    required=False, type=int, default=2000)

args = parser.parse_args()

run_modes = ['train', 'test', 'validation']
for run_mode in run_modes:

    threads = args.nthreads

    classes = []
    if args.classes[-3:] == 'txt':
        classes = [cl.strip() for cl in open(args.classes).readlines()]
        print(classes)
    else:
        for class_name in args.classes.split(','):
            classes.append(class_name)

    with open('csv_files/class-descriptions-boxable.csv', mode='r') as infile:
    ...
    for ind in range(0, len(classes)):
        ...
        command = "grep " + \
            ...
            " csv_files/" + run_mode + "-annotations-bbox.csv"
        ...
        for line in class_annotations:
            ...
            if (len(set(commands))) % (args.max) == 0:
                break
