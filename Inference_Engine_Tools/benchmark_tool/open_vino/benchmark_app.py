...
from utils.inputs_filling import get_inputs
from utils.logging import logger
from utils.progress_bar import ProgressBar
from utils.utils import next_step, read_network, config_network_inputs, get_number_iterations, \
    ...
    try:
        ...
        if args.report_type:
            ...
            statistics.add_parameters(
                StatisticsReport.Category.COMMAND_LINE_PARAMETERS, get_command_line_arguments(sys.argv))
        ...
        benchmark.add_extension(args.path_to_extension,
        ...
        logger.info('Network batch size: {}, precision: {}'.format(
            ie_network.batch_size, ie_network.precision))
        ...
        exe_network = benchmark.load_network(
            ie_network, perf_counts, args.number_infer_requests)
        duration_ms = "{:.2f}".format(
            (datetime.now() - start_time).total_seconds() * 1000)
        if statistics:
        ...
        benchmark.niter = get_number_iterations(
            benchmark.niter, len(exe_network.requests), args.api_type)
        ...
        requests_input_data = get_inputs(
            path_to_input, batch_size, ie_network.inputs, infer_requests)
        ...
        if statistics:
            statistics.add_parameters(StatisticsReport.Category.RUNTIME_CONFIG,
                                      [
                                          ...
                                          ('number of iterations', str(
                                              benchmark.niter) if benchmark.niter else "0"),
            ...
            for nstreams in benchmark.device_number_streams.items():
                statistics.add_parameters(StatisticsReport.Category.RUNTIME_CONFIG,
                                          [
                                              ("number of {} streams".format(
                                                  nstreams[0]), str(nstreams[1])),
                                          ])
        ...
        progress_bar = ProgressBar(
            progress_bar_total_count, args.stream_output, args.progress)
        ...
        if statistics:
            statistics.add_parameters(StatisticsReport.Category.EXECUTION_RESULTS,
                                      [
                                          ('total execution time (ms)', '{:.2f}'.format(
                                              get_duration_in_milliseconds(total_duration_sec))),
