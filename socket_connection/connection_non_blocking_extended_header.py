...
class RaspyConnection:
    ...
    def start_server(self, address='10.10.73.89', port=5001):
        ...
        self.clients = {}  # hier schon self.my_socket hinzufügen
    ...
    def receive_data(self):
        ...
        for notified_socket in self.notified_sockets:
            ...
            if notified_socket == self.my_socket and self.type == 'server':
                ...
                # Receive
                self.address = notified_socket.recv(self.HEADER_LENGTH / 3)
                if len(self.address) == 0:
                    self.socket_list.remove(notified_socket)
                    continue
                self.address = self.address.decode('utf-8')
                self.data_type = notified_socket.recv(
                    self.HEADER_LENGTH / 3).decode('utf-8')
                self.data_length = int(notified_socket.recv(
                    self.HEADER_LENGTH / 3).decode('utf-8'))

                if self.data_type == 'image':  # hier mit data type im header arbeiten
                    # receive image
                    self.data = b''
                    ...
                    self.datas[self.address] = [self.data_type, self.data]
                    print('data received')

        return self.datas

    # arg: list of sockets and die gesendet werden soll
    def send_data(self, data, data_type, adresses=None):
        ...
        for sender_socket in self.sender_sockets:
            ...
            if adresses == None or self.clients[sender_socket] in adresses:
                ...
                if data_type == 'image':
                    ...
                    self.data = np.array(self.img_encode).tostring()
                ...
                self.data_length=f'{len(self.data):<{self.HEADER_LENGTH / 3}}'.encode(
                        'utf-8')
                self.data_type=f'{data_type:<{self.HEADER_LENGTH / 3}}'.encode(
                    'utf-8')
                self.address=f'{self.my_socket}
